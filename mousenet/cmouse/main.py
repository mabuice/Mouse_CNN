...
parser.add_argument('-data', metavar='DIR',
                    help='path to dataset', default=IMAGENET_DIR)
...
parser.add_argument('--fixmask',  default=0, type=int, help='if use Gaussian mask')
args = parser.parse_args()

SEED = args.seed
MASK = args.mask
FIXMASK = args.fixmask
...
def main():
    ...
    if USE_WANDB:
        ...
        config.log_interval = 100     # how many batches to wait before logging training status
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #     model = MouseNetCompletePool(net, mask=MASK)
        ...
        print('vgg16 model')
        arch = 'vgg16'
        model = torchvision.models.vgg16(pretrained=False, init_weights=True)
    else:
        #create model
        if args.pretrained:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if USE_WANDB:
            ...
            if i % config.log_interval == config.log_interval-1:    # print every config.log_interval mini-batches
                wandb.log({
                    "running loss": running_loss/config.log_interval,
                    "running acc1": top1.avg,
                    "running acc5": top5.avg,
                    })
                running_loss = 0.0
...
def save_checkpoint(state, is_best):
    ...
    filename=RESULT_DIR+'/'+RUN_NAME_MASK+'/%s_%s_%s_%s.pth.tar'%(FIXMASK, args.seed, state['epoch'], state['best_acc1']) 
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)

    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, RESULT_DIR+'/'+RUN_NAME_MASK+'/%s_%s_model_best.pth.tar'%(FIXMASK, args.seed))
...
def adjust_learning_rate(optimizer, epoch, args):
    """Sets the learning rate to the initial LR decayed by 10 every LR_EPOCHS epochs"""
    if epoch // LR_EPOCHS < 1:
        lr = args.lr
    elif epoch // LR_EPOCHS >= 1:
        lr = args.lr * 0.1
        if epoch >  LR_EPOCHS + LR_EPOCHS2:
            lr = args.lr * 0.1 * 0.1
    # lr = args.lr * (0.1 ** (epoch // LR_EPOCHS))
